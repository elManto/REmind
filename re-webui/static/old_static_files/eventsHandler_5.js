...
function get_xrefs_from(func) {
    $.post(base_url + "/fifth_chall/get_xrefs_from", data={'name':func, 'token':id}, function(data) {
...
function get_xrefs_to(func) {
    $.post(base_url + "/fifth_chall/get_xrefs_to", data={'name':func, 'token':id}, function(data) {
                xref = JSON.parse(data);
...
function downloadCallGraph() {
    $.post(base_url + "/fifth_chall/getCallGraph", data={'name':'callgraph'}, function(data) {
...
function downloadCFG(func) {
        ...
        $.post(base_url + "/fifth_chall/getCFG", data={'name':func, 'token':id}, function(data) {
...
function send_results(data) {
    ...
    $.get( base_url + "/fifth_chall/getmethod/" + id + ";" + JSON.stringify(data));
...
function rename(htmlElementToRename) {
    ...
    var r = prompt("insert the new name");
    if(r == "" || r == null)	return;
    if(r.length > 8) {
        r = r.substring(0, 8)
    }
...
function downloadData() {
	var downloadedData;	
	$.get(base_url + "/fifth_chall/getevents/?id=" + id + "&&fcn_name=" + func_to_download,  function(data) {
	    myData = data;
