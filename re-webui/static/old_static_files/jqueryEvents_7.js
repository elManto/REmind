...
$(document).ready(function() {
    ...
    $("div.cfg").on("mouseover", function() {
        ...
        // set ondblclick events over 'jumpable' addresses (Note that these are 'local' jumps in the function)
	...
	$('span.token.generic').each(function() {
		if((this).innerHTML.includes("call") && $(this).attr("binded") != "true") {
			...
			$(this).on("dblclick", function(event) {
				...
				push_function_history(func_to_download, addr)
			    	//localStorage.setItem("myFunctionHistory", func_to_download);
			    	//localStorage.setItem("myAddrHistory", localStorage.getItem("addr"))
				window.location.replace(base_url + '/seventh_chall/?function=' + foo )
			});
	    ...
	    selector = state + '.node'
            if($('g#' + selector).attr("binded") != "true") {
                $('g#' + selector).attr("binded", "true");
                $('g#' + selector).on("mouseenter", function(){
                    selected(this);
            });
            }
        });
