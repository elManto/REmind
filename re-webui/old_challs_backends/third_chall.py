...
bp = Blueprint('third_chall', __name__, url_prefix='/third_chall')
user = ()


config = configparser.ConfigParser()
config.read(os.path.dirname(os.path.realpath(__file__)) + '/config.ini')
challenge_num = config['chall3'].getint('challenge_num')
solution = config['chall3']['solution']
...
def third_chall():
    ...
    if session['status_solutions'][challenge_num] == 1:
        session['speed_solved'] = True
    return render_template("third_chall.html")
...
def getBlock():
    if (request.method == 'POST'):
        ...
        db = get_db()
        user = read_query(db,
            'SELECT * FROM user WHERE tmp_token = %s', (tmp_token,)
        )[0]
        if user is None:
        ...
        files = getFiles(absolutePath + '/third_json/')
        if (func_required + '.json' in files):
            f = open(absolutePath + '/third_json/' + func_required + '.json')
            out = f.read()
